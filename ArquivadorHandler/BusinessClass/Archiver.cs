...
namespace ArquivadorHandler.BusinessClass
{
    ...
    {
        //private static readonly log4net.ILog logger = log4net.LogManager.GetLogger("Handler.Archiver.cs");

        ArquivadorHandler.Config.ArchiverConfig _config;
        public Config.ArchiverConfig Config
        { get { return _config; } }
        ...
        /// Construtor
        /// </summary>
        public Archiver(Config.ArchiverConfig config)
        {
            _config = config;
            LoadDestination();
        }
        ...
        {
            bool retorno = false;
            if (dir.FullName == _config.Dir.Info.FullName)
            { retorno = true; }
        ...
        {
            string path = Path.Combine(_config.Dir.Info.FullName, DateTime.Now.ToString("yyyy-MM-dd"));
        ...
        public void CleanOldDirectories()
        {
            ...
            dirList.AddRange(_config.Dir.Info.GetDirectories());
        ...
        {
            bool retorno = false;
            if (dir.CreationTime.AddDays(_config.Days) < DateTime.Now)
            { retorno = true; }
